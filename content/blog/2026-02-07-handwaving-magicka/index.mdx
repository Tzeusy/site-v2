...
We decided to emulate an Xbox controller for the task, as it allows for full 360-degree navigation (mouse/keyboard are tied down to 8 directions via WASD). Existing bindings existed for emulation of Xbox controls, meaning we wouldn't need to interface with the game itself, only the OS; the game 'simply sees' an Xbox controller. This was done leveraging the [evdev](https://python-evdev.readthedocs.io/en/latest/) library.
